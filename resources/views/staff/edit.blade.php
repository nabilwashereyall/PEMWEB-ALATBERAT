...
<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('staff.update', $staff->IdStaff) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('NamaStaff') is-invalid @enderror" 
                           id="NamaStaff" name="NamaStaff" value="{{ old('NamaStaff', $staff->NamaStaff) }}" required>
                    @error('NamaStaff')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('NoTelepon') is-invalid @enderror" 
                           id="NoTelepon" name="NoTelepon" value="{{ old('NoTelepon', $staff->NoTelepon) }}">
                    @error('NoTelepon')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('Alamat') is-invalid @enderror" 
                              id="Alamat" name="Alamat" rows="3">{{ old('Alamat', $staff->Alamat) }}</textarea>
                    @error('Alamat')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
